...
def handle_webhook(payload: bytes, sig_header: str, db) -> dict:
    """
    Verifies Stripe signature via construct_event(), then parses the raw
    JSON payload directly as a plain Python dict — no StripeObject quirks.
    """
    try:
    ...
    # Parse raw bytes as a plain dict — sidesteps all StripeObject .get() issues
